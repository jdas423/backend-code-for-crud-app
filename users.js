...
router.get("/",wrapAsync(async(req,res,next)=>{
    ...
    if(!req.query.hasOwnProperty("pages")) throw new appError(500,"Invalid query");
    next();  
}))
...
router.get("/",wrapAsync(async(req,res,next)=>{
    ...
    if(pages<1){
        throw new appError(500,"Invalid Page Number");
    }
    const sInd= (pages-1)*10+1;
    const eInd= pages*10;   
    const users= await User.find({$and : [{id: {$gte: sInd}},{id: {$lte: eInd}}]});
    if(!users.length) throw new appError(400,"page Not Found");
